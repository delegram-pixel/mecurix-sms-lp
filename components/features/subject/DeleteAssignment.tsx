...
export default function DeleteAssignment({
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ["assignment", assignment.id],
    mutationFn: async () => {
      const token = await getCookie("token");
      await axios.delete(
        `/school/${schoolId}/subject/${subjectId}/assignment/${assignment.id}`,
    ...
    onError: (err: AxiosError) => {
      toast({
        ...
        description:
          ...
          `Failed to delete "${assignment.title}". Please try again.`,
        variant: "destructive",
      });
    },
    onSettled: () => {
    ...
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This will permanently delete the "{assignment.title}" assignment and
